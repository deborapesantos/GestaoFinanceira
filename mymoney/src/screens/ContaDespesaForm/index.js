...
salvar(){
  ...
  traduzirDespesaParaTela(){
    this.setState({ Titulo:this.state.tabAux.receita.Titulo});
    this.setState({ DataPagamento:this.state.tabAux.receita.DataPagamento});
    this.setState({ IsFixo:this.state.tabAux.receita.IsFixo});
    this.setState({ DataFixaVencimento:this.state.tabAux.receita.DataFixaVencimento});
    this.setState({ IsParcelado:this.state.tabAux.receita.IsParcelado});
    this.setState({ DataVencimento:this.state.tabAux.receita.DataVencimento});
    this.setState({ ValorTotal:this.state.tabAux.receita.ValorTotal});
    this.setState({ Pago:this.state.tabAux.receita.Pago});
    this.setState({ CadContaId:this.state.tabAux.receita.CadContaId});
    this.setState({ CodigoTabTipoDespesa:this.state.tabAux.receita.CodigoTabTipoDespesa});
    this.setState({ CadGrupoFamiliarId:this.state.tabAux.receita.CadGrupoFamiliarId});

    this.state.contaSelecionada =  this.state.tabAux.ListaConta.filter(x=>x.id == this.state.CadContaId);
    this.state.categoriaSelecionada =  this.state.tabAux.ListaTipoDespesa.filter(x=>x.id == this.state.CodigoTabTipoDespesa);
    this.state.grupoSelecionado =  this.state.tabAux.ListaGrupo.filter(x=>x.id == this.state.CadGrupoFamiliarId);
  }
