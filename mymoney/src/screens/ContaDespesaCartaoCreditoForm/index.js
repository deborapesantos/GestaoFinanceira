...
export default class ContaDespesaCartaoCreditoForm extends React.Component {
  onGoFocus() {
  ...
  constructor(props) {
    ...
    this.state={
        ...
        faturaCartaoCreditoSelecionada:{
          text:null,
          id:null
        },
        cartaoCreditoSelecionada:{
          text:null,
          id:null
        },
        valorParcela:0
...
criarNovo(){
  ...
  let model   = {
      ...
      CadGrupoFamiliarId:null,
      CadUsuarioId:null,
      CadFaturaCartaoCreditoId:this.state.CadFaturaCartaoCreditoId,
...
console.log(model);
  ...
  selecionarCartaoCredito(param, index){
    if(param){
     var filtrado = this.state.tabAux.ListaFaturaCartaoCredito.filter(x=>x.idFilter === param.id);
     this.setState({ listaFaturaFiltrada:filtrado });
     var indexi = index+1;
      if(indexi != this.state.tabAux.ListaCartaoCredito.length){
        this.setState({ cartaoCreditoSelecionada: param})
      }
    }

  }

  selecionarFatura(param, index){
    if(param){
      var indexi = index+1;
      if(indexi != this.state.tabAux.ListaFaturaCartaoCredito.length){
        this.setState({ CadFaturaCartaoCreditoId: param.id})
        this.setState({ faturaCartaoCreditoSelecionada: param})
      }
    }
  ...
  parcelar(valor){
    var valorTotal = Number(this.state.ValorTotal.toString().replace("R$",""))
    this.setState({QdteParcelas:valor })
    if(valorTotal > 0 && this.state.QdteParcelas > 0){
      this.state.valorParcela = valorTotal / this.state.QdteParcelas;
    }else{
      //apresenta mensagemd e erro
    }
    ...
    return (
      ...
      <Container style={[Style.container,{backgroundColor:'#ffffff'}]}>
        ...
        <View style={Style.row}>
          <View style={Style.col1,styles.inputView} >
          <Item inlineLabel>
              <Label style={styles.labelText}>Data da compra</Label>
        ...
        <View style={[Style.row]} >
          ...
          onPress={() =>
            ActionSheet.show(
              ...
              buttonIndex => {
                this.selecionarCartaoCredito(this.state.tabAux.ListaCartaoCredito[buttonIndex],buttonIndex);
              }
            )}
          >
            {
              this.state.cartaoCreditoSelecionada?.text == null ?
              <Text>Escolha uma opção</Text>
              :
              <Text>{this.state.cartaoCreditoSelecionada?.text}</Text>
              }
        </TouchableOpacity>
        </Item>
        </View>
      ...
      <View style={[Style.row]} >
          ...
          onPress={() =>
            ActionSheet.show(
              {
                options: this.state.listaFaturaFiltrada,
                cancelButtonIndex: this.state.listaFaturaFiltrada.length,
                destructiveButtonIndex: this.state.listaFaturaFiltrada.length,
                title: "Escolha uma fatura"
              },
              buttonIndex => {
                this.selecionarFatura(this.state.listaFaturaFiltrada[buttonIndex],buttonIndex);
              }
            )}
          >
            {
              this.state.faturaCartaoCreditoSelecionada?.text == null ?
              <Text>Escolha uma opção</Text>
              :
              <Text>{this.state.faturaCartaoCreditoSelecionada?.text}</Text>
              }
        </TouchableOpacity>
        </Item>
        </View>
      ...
      <View style={Style.row}>
          <View style={Style.col1,styles.inputView} >
          <Item inlineLabel>
              <Label style={styles.labelText}>Parcelar despesa?</Label>
              <Switch
                  value={this.state.IsParcelado}
                  testID="fixo"
                  onValueChange={(value)=> this.setState({IsParcelado: value})}/>
            </Item>
          ...
          {
            this.state.IsParcelado ?

                <View>
                <View style={Style.row}>
                    <View style={Style.col1,styles.inputView} >
                    <Item inlineLabel>
                        <Label style={styles.labelText}>Em quantas vezes?</Label>
                        <TextInput  
                        style={styles.inputText}
                        placeholder="ex. 12"
                        keyboardType="number-pad"
                        placeholderTextColor="#d3d3d3"
                        onChangeText={valor =>  this.parcelar(valor) }/>
                      </Item>
                    </View>
                  </View>
                  ...
                  {
                    this.state.QdteParcelas > 0 ?
                    <View style={Style.row}>
                    <View style={Style.col1,styles.inputView} >
                    <Item inlineLabel>
                        <Label style={styles.labelText}>Valor da parcela</Label>
                          <Text>{this.state.valorParcela}</Text>
                      </Item>
                    </View>
                  </View>
                  :
        ...
        <View style={Style.row}>
          <View style={Style.col1,styles.inputView} >
          <Item inlineLabel>
              <Label style={styles.labelText}>Despesa recorrente?</Label>
              <Switch
...

          {
            this.state.IsFixo ?
                ...
                <View style={Style.row}>
                          <View style={Style.col1,styles.inputView} >
                          <Item inlineLabel>
                              <Label style={styles.labelText}>Repetir em quantas faturas?</Label>
                              <TextInput  
                              style={styles.inputText}
                              placeholder="ex. 12"
                              keyboardType="number-pad"
